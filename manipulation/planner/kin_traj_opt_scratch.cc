...
DEFINE_double(duration_weight, 1, "Weight on squared jerk cost.");
DEFINE_double(
    center_x, 0.2,
...
int DoMain() {
  ...
  prog.AddLinearConstraint(prog.duration() >= Vector1<double>(0.5),
                           {{0.0, 0.0}});
  ...
  if (max_velocity > 0) {
    prog.AddLinearConstraint(
        prog.velocity() <= Vector2<double>::Constant(max_velocity) * prog.duration(),
        {{0.0, 1.0}});
    prog.AddLinearConstraint(
        prog.velocity() >= Vector2<double>::Constant(-max_velocity) * prog.duration(),
        {{0.0, 1.0}});
  ...
  if (duration_weight > 0) {
    prog.AddQuadraticCost(prog.duration().transpose() * prog.duration());
  }
