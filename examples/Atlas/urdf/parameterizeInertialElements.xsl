<xsl:stylesheet version="2.0" 
  ...
  <!--Anything not otherwise specified should be passed through-->
  ...
  <xsl:template match="link">
    <xsl:call-template name="inertial-to-parameters">
      <xsl:with-param name="inertial" select="inertial"/>
  ...
  <xsl:template match="mass">
    <xsl:copy>
      <xsl:attribute name="value" select="concat('$m__',../../@name)"/>
    </xsl:copy>
  </xsl:template>
  ...
  <!--<xsl:template match="inertia">-->
    <!--<xsl:copy>-->
      <!--<xsl:for-each select="@*">-->
        <!--<xsl:attribute name="{name()}" select="concat('$',name(),'__',../../../@name)"/>-->
      <!--</xsl:for-each>-->
    <!--</xsl:copy>-->
  <!--</xsl:template>-->

  <xsl:template match="inertial/origin">
    <xsl:variable name="linkname" select="../../@name"/>
    <xsl:copy>
      <!--<xsl:attribute name="xyz" select="concat('$com_x__',$linkname,' ','$com_y__',$linkname,' ','$com_z__',$linkname)"/>-->
      <xsl:attribute name="xyz" select="concat('$com_x__',$linkname,' 0 ','$com_z__',$linkname)"/>
    </xsl:copy>
  </xsl:template>

  <!--inertial ==> parameters-->
  <xsl:template name="inertial-to-parameters">
    <xsl:param name="inertial"/>
    <xsl:param name="linkname" as="xs:string"/>
    <xsl:call-template name="link-value-to-parameter">
      <xsl:with-param name="param-name" select="'m'"/>
      <xsl:with-param name="linkname" select="$linkname"/>
      <xsl:with-param name="value" select="$inertial/mass/@value"/>
    </xsl:call-template>
    <xsl:call-template name="link-xyz-to-parameters">
      <xsl:with-param name="param-name" select="'com'"/>
      <xsl:with-param name="linkname" select="$linkname"/>
      <xsl:with-param name="value" select="$inertial/origin/@xyz"/>
    </xsl:call-template>
    <!--<xsl:call-template name="inertia-to-parameters">-->
      <!--<xsl:with-param name="inertia" select="$inertial/inertia"/>-->
      <!--<xsl:with-param name="linkname" select="$linkname"/>-->
    <!--</xsl:call-template>-->
  </xsl:template>

  <xsl:template name="inertia-to-parameters">
    <xsl:param name="inertia"/>
    <xsl:param name="linkname" as="xs:string"/>
    <xsl:for-each select="$inertia/@*">
      <xsl:call-template name="link-value-to-parameter">
  ...
  <xsl:template name="link-xyz-to-parameters">
    <xsl:param name="param-name" as="xs:string"/>
    <xsl:param name="linkname" as="xs:string"/>
    <xsl:param name="value" as="xs:string"/>
    <xsl:variable name="Vxyz" select="mat:V($value)"/>
    <xsl:call-template name="link-value-to-parameter">
      <xsl:with-param name="param-name" select="concat($param-name,'_x')"/>
      <xsl:with-param name="linkname" select="$linkname"/>
      <xsl:with-param name="value" select="$Vxyz[1]"/>
    </xsl:call-template>
    <!--<xsl:call-template name="link-value-to-parameter">-->
      <!--<xsl:with-param name="param-name" select="concat($param-name,'_y')"/>-->
      <!--<xsl:with-param name="linkname" select="$linkname"/>-->
      <!--<xsl:with-param name="value" select="$Vxyz[2]"/>-->
    <!--</xsl:call-template>-->
    <xsl:call-template name="link-value-to-parameter">
      <xsl:with-param name="param-name" select="concat($param-name,'_z')"/>
      <xsl:with-param name="linkname" select="$linkname"/>
      <xsl:with-param name="value" select="$Vxyz[3]"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template name="link-value-to-parameter">
    <xsl:param name="param-name" as="xs:string"/>
    <xsl:param name="linkname" as="xs:string"/>
    <xsl:param name="value" as="xs:double"/>
    <xsl:call-template name="value-to-parameter">
      <xsl:with-param name="name" select="concat($param-name,'__',$linkname)"/>
      <xsl:with-param name="value" select="$value"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template name="value-to-parameter">
