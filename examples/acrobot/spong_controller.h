...
class AcrobotSpongController : public systems::LeafSystem<T> {
 public:
  AcrobotSpongController()
    ...
    acrobot_double.GetInputPort("elbow_torque").FixValue(acrobot_context_double.get(), 0.0);
  ...
  const SpongControllerParams<T>& get_parameters(
    ...
    return this->template GetNumericParameter<SpongControllerParams>(context, 0);
  }
