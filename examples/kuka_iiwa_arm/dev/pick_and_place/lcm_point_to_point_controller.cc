...
class PlanToPoseConverter : public systems::LeafSystem<double> {
 public:
  PlanToPoseConverter() {
    this->DeclareAbstractInputPort();
    this->DeclareAbstractOutputPort(Isometry3<double>::Identity(),
                                    &PlanToPoseConverter::CalcPoseOutput);
  }

 private:
  void CalcPoseOutput(const systems::Context<double>& context,
                      Isometry3<double>* pose) const {
    ...
    if (plan_input.plan.empty()) {
      ...
      *pose = DecodePose(plan_input.plan.front().pose);
    }
  }
};
...
std::unique_ptr<RigidBodyTree<double>> BuildTree(
  ...
  if (add_grasp_frame) {
    ...
    robot->addFrame(std::make_shared<RigidBodyFrame<double>>(kGraspFrameName, robot->FindBody(configuration.end_effector_name), X_EG));
    robot->compile();
  }
...
LcmPointToPointController::LcmPointToPointController(
  ...
  // Add a block to extract the pose of the first state in a robot_plan_t.
  auto plan_to_pose_converter = builder.AddSystem<PlanToPoseConverter>();
  ...
  desired_end_effector_pose_input_port_ = builder.ExportInput(
      plan_to_pose_converter->get_input_port(0));
  ...
  builder.Connect(
      plan_to_pose_converter->get_output_port(0),
