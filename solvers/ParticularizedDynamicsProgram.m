classdef ParticularizedDynamicsProgram < MixedIntegerConvexProgram
  properties
    N = 2
    position_max = 100
    velocity_max = 10
  ...
  methods
    function obj = ParticularizedDynamicsProgram(edge_length, N, dt, shape)
    ...
    function obj = setInnerApproxShape(obj, shape)
      switch shape
        ...
        case 'dodecahedron'
          obj.n_sectors = 12;
          phi = (1+sqrt(5))/2;
          obj.center_to_face = sqrt(3)*phi/(3*sqrt(3-phi));
    ...
    function obj = addEdgeLengthConstraints(obj)
      ...
      for k = 1:size(obj.E,2)
        ...
        for n = 1:obj.N
          ...
          rj = obj.vars.(obj.positionName(j, n)).symb;
          gij = obj.vars.(obj.forceName(i, j, n)).symb;
          obj = obj.addSymbolicConstraints((rj-ri)'*(rj-ri) <= (1.01*obj.edge_length(k))^2);
    ...
    function xtraj = extractXtraj(obj)
      x_data = zeros(12*4, obj.N);
      t = cumsum(repmat(obj.dt, 1, obj.N));
      for n = 1:obj.N
        positions = zeros(6, 4);
        velocities = zeros(6, 4);
        for i = 1:4
    ...
    function plotAngularMomentum(obj)
      ...
      r_data_3D = reshape(r_data, [3,4,obj.N]);
      v_data_3D = reshape(v_data, [3,4,obj.N]);
  ...
  methods (Static)
    ...
    function obj = fromVertices(vertices, varargin)
      ...
      obj = ParticularizedDynamicsProgram(edge_length, varargin{:}); %#ok
      for i = 1:4
        for n = 1:obj.N
