...
leg_length = 0.1;
for i = 1:M
  prog = LeggedLocomotionPlanner(I, m, N, dt, z_fixed_array, w_fixed_array, F_fixed_array, M_fixed_array, false);
  prog.foot_mass = 0.2;
  prog.velocity_max = 2;
 ...
 [prog, solvertime, objval] = prog.solveGurobi(params);
  ...
  for j = 1:numel(prog.feet)
    r_foot_new = zeros(3, prog.N);
    for n = 1:prog.N
      r_foot_new = quatRotateVec(quatConjugate(prog.vars.z.value(:,n)), ...
                                 prog.vars.(sprintf('r_foot%d',j)).value(:,n) - ...
                                   prog.vars.r.value(:,n));
      delta_norm = delta_norm + sum(r_foot_fixed_array(:, n, j) - r_foot_new);
      r_foot_fixed_array(:,:,j) = (1-lam)*r_foot_fixed_array(:, n, j) + lam*r_foot_new;
    end
