function [q_traj,info,v,V,parent] = ikRRT2(goal_bias,options)
...
var_names = r.getPositionFrame().coordinates;
problem = MotionPlanningProblem(nq,var_names);
...
problem = problem.addConstraint(collision_constraint_2{1});
...
options.display_fcn = @handPosition;
...
rrt_time = tic;
[xtraj,info,V,parent] = problem.rrt(q_start,q_end,@sampleFunction,options);
toc(rrt_time);
...
%else
  %q_traj = []
%end
  function handPosition(V,parent,last_drawn_edge_num)
    
  end
function [x_interp,valid_interp] = ikInterpolation(x1,x2,alpha)
  q_nom_local = (1-alpha)*x1+alpha*x2;
%   q_seed = x1;
  persistent q_seed x2_prev
  if (isempty(x2_prev) || all(x2 ~= x2_prev)) || isempty(q_seed), q_seed = x1; end
  [q, info, infeasible_constraint] = inverseKin(r, q_seed, q_nom_local, base_constraints{:}, ikoptions);
...
function q_sample = sampleFunction()
  q_sample = (jlmax-jlmin).*rand(nq,1) + jlmin;
%   [q_sample, info, infeasible_constraint] = inverseKin(r, q_nom, q_sample, base_constraints{:},collision_constraint, ikoptions);
end
end
