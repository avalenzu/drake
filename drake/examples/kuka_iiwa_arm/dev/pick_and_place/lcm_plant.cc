...
LcmPlant::LcmPlant(
  ...
  for (int i = 0; i < num_robot_bases; ++i) {
    ...
    const std::string robot_base_name =
        tree.get_body(tree.FindBaseBodies(robot_base_instance_id).front())
            .get_name();
    RigidBody<double>* body =
        tree.FindBody(robot_base_name, "", robot_base_instance_id);
    frames.emplace_back(new RigidBodyFrame<double>(
        "robot_base_" + std::to_string(i), body,
  ...
  for (int i = 0; i < num_objects; ++i) {
    ...
    const std::string object_name =
        tree.get_body(tree.FindBaseBodies(object_instance_id).front())
            .get_name();
    RigidBody<double>* body =
        tree.FindBody(object_name, "", object_instance_id);
    frames.emplace_back(new RigidBodyFrame<double>(
        "object_" + std::to_string(i), body,
  ...
  for (int i = 0; i < num_tables; ++i) {
    ...
    const std::string table_name =
        tree.get_body(tree.FindBaseBodies(table_instance_id).front())
            .get_name();
    RigidBody<double>* body = tree.FindBody(table_name, "", table_instance_id);
    frames.emplace_back(new RigidBodyFrame<double>(
        "table_" + std::to_string(i), body,
