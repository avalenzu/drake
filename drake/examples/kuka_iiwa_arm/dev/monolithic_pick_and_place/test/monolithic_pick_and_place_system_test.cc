...
// TODO(avalenzu): Take this out once the test works.
...
const std::string kIiwaPath =
    "drake/manipulation/models/iiwa_description/urdf/"
    "iiwa14_polytope_collision.urdf";
const std::string kEndEffectorName = "iiwa_link_ee";
const std::string kYellowPostPath =
    "drake/examples/kuka_iiwa_arm/models/objects/yellow_post.urdf";
const std::string kExtraHeavyDutyTablePath =
    "drake/examples/kuka_iiwa_arm/models/table/"
    "extra_heavy_duty_table_surface_only_collision.sdf";
// const std::string kTargetPath =
//"drake/examples/kuka_iiwa_arm/models/objects/big_robot_toy.urdf";
// const Vector3<double> kTargetDimensions{0.1, 0.035, 0.18};
const std::string kTargetPath =
...
class SingleArmSingleMoveTests
...
TEST_P(SingleArmSingleMoveTests, FinalPoseTest) {
  ...
  plant->Initialize(&sys->GetMutableSubsystemContext(*plant, context));
  ...
  //   - M: Model frame of "place" table (origin of the URDF/SDF model)
  ...
  Eigen::Vector3d X_TO_expected_rpy = math::rotmat2rpy(X_TO_expected.linear());
  ...
  const double angle_tolerance = 5.0 * M_PI / 180;
  ...
  EXPECT_TRUE(CompareMatrices(object_rpy, X_TO_expected_rpy, angle_tolerance));
  EXPECT_TRUE(CompareMatrices(object_velocity.head(3), Vector3<double>::Zero(),
                              angular_velocity_tolerance));
  EXPECT_TRUE(CompareMatrices(object_velocity.tail(3), Vector3<double>::Zero(),
                              linear_velocity_tolerance));
}

INSTANTIATE_TEST_CASE_P(SelectedPairs, SingleArmSingleMoveTests,
...
INSTANTIATE_TEST_CASE_P(InitialTable0, SingleArmSingleMoveTests,
                        ::testing::Combine(::testing::Values(0),
                                           ::testing::Values(1, 2, 3, 4, 5)));
INSTANTIATE_TEST_CASE_P(InitialTable1, SingleArmSingleMoveTests,
                        ::testing::Combine(::testing::Values(1),
                                           ::testing::Values(0, 2, 3, 4, 5)));
INSTANTIATE_TEST_CASE_P(InitialTable2, SingleArmSingleMoveTests,
                        ::testing::Combine(::testing::Values(2),
                                           ::testing::Values(0, 1, 3, 4, 5)));
INSTANTIATE_TEST_CASE_P(InitialTable3, SingleArmSingleMoveTests,
                        ::testing::Combine(::testing::Values(3),
                                           ::testing::Values(0, 1, 2, 4, 5)));
INSTANTIATE_TEST_CASE_P(InitialTable4, SingleArmSingleMoveTests,
                        ::testing::Combine(::testing::Values(4),
                                           ::testing::Values(0, 1, 2, 3, 5)));
INSTANTIATE_TEST_CASE_P(InitialTable5, SingleArmSingleMoveTests,
