...
struct PickAndPlaceStateMachineSystem::InternalState {
  InternalState(const std::string& iiwa_model_path,
      ...
      : world_state(iiwa_model_path, end_effector_name, table_radii,
                    object_dimensions),
        state_machine(table_radii.size() > 1),
...
PickAndPlaceStateMachineSystem::PickAndPlaceStateMachineSystem(
  ...
  input_port_table_state_.resize(table_radii_.size());
  for (int i = 0; i < static_cast<int>(table_radii_.size()); ++i) {
