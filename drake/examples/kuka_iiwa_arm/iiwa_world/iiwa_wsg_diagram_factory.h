 ...
 public:
  ...
  IiwaAndWsgPlantWithStateEstimator(
      ...
      : IiwaAndWsgPlantWithStateEstimator(
            ...
            std::vector<ModelInstanceInfo<T>>{iiwa_info},
            std::vector<ModelInstanceInfo<T>>{wsg_info},
            std::vector<ModelInstanceInfo<T>>{box_info}){};
  ...
  const systems::OutputPort<T>& get_output_port_iiwa_state(int index = 0) const {
