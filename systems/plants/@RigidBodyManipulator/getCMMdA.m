function [A,dA] = getCMMdA(model,kinsol,qd)
  ...
  for i = m.NB:-1:1
    ...
    if m.parent(i) > 0
      ...
      if nargout > 1
        ...
        dIc_dq{m.parent(i)} = dIc_dq{m.parent(i)} + (dXup_dq{i}'*Ic{i} + Xup{i}'*dIc_dq{i})*Xup{i} + Xup{i}'*Ic{i}*dXup_dq{i};
      end
    end
  end
  ...
  for i = 1:m.NB
    ...
    if m.parent(i) > 0
      ...
      if nargout > 1
        ...
        dXworld_dq{i} = dXup_dq{i}*Xworld{m.parent(i)} + Xup{i}*dXworld_dq{m.parent(i)};
      end      
    else
      Xworld{i} = Xup{i};
      if nargout > 1
    ...
    if nargout > 1
      ...
      dXg_dq{i} = dXworld_dq{i} * Xcom + Xworld{i} * dXcom_dq(:,:,i); 
