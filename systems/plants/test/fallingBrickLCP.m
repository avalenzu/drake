function fallingBrickLCP(N)
  ...
  for i = 1:N
    p = p.clearStoredContacts();
    p_points = p_points.clearStoredContacts();
    x0 = p.resolveConstraints([0;0;1;pi*(2*rand(3,1)-1);0.2*(2*rand(3,1)-1);min(0.1*pi,max(-0.1*pi,randn(3,1)))]);
    ...
    tf = 4;
    try
      p_points.simulate([0 tf],double(x0));
    catch ex
      continue; % Initial conditions were too much for path
    end
    timer_points = tic;
